...
# Convention: log.md is append-only, with each entry on its own line
# starting with a `[timestamp]` prefix (ISO-8601 timestamps sort
# lexicographically, which is what we rely on here).
#
# Strategy:
#   1. Pull any non-entry header lines (those not starting with `[`)
